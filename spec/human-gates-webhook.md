...
What it does not close is three things. A host that lies about what it is asking approval for lies before any of this is written (§10). A genuine decision can still be relabelled afterwards as answering a different call, because excluding the arguments leaves nothing to recompute the digest from (§10). And this check reaches only gates this channel decided: it reads `decided_by` to know which those are, and §8.3 already states that a terminal approval signs nothing and leaves no artifact to re-verify.
