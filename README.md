...
| **Audit log** | JSONL per agent per UTC day. With `identity.did` set, every entry is Ed25519-signed and hash-chained; `constle audit verify` detects tampering and reports the offending line. A gate decided over the human-gates webhook also records the approver's signed decision, so `constle audit verify --agentfile=…` can show that the approver really did approve a given call — an intact chain proves only that the runtime's own account of a run is unaltered, not that it is true. | Shipped |
...
| `constle audit verify [--agentfile=<path>] <logfile>` | Verify an audit log's signatures and hash chain, and re-verify the human-gate decisions it records. `--agentfile` takes both trust anchors from the Agentfile; `--did=` and `--approver-pubkey=` pin them individually |
